...
public class SelectFolderViewController: UIViewController {
    ...
    var folderList: [VocaForAll] = []
    let selectedGameType: GameType

    var selectedIndexList: [Int] = []

    let wordList: PublishSubject<[WordDownload]>

    let disposeBag = DisposeBag()
    
    public init(_ folderList: [VocaForAll], gameType: GameType) {
    ...
    @objc func confirmDidTap(_ sender: UIButton) {
        var words = [WordDownload]()

        if let folderCoreDataList = folderList as? [VocaForAll] {
    ...
    func updateConfirmButton() {
        confirmButton.isEnabled = selectedIndexList.isEmpty ? false : true
